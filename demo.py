...
def get_rec_point(file):
	...
	for line in lines:
		...
		line = line.replace('$hole$','unknown_api()')
		recobj=re.findall('[a-zA-Z0-9_\.\[\]]+\.[a-zA-Z0-9\_]+\(.*\)',line)
		rec=recobj[0]
		caller=get_caller(rec)
		callee=rightans[testnum]
		...
		i=0
		latest_line=line.replace('$hole$','unknown_api()')
		...
		print(file+'#'+str(lno)+'#'+caller+'::'+ft+'#'+typec)
		#sys.exit()
...
#获取testcase对应的原始目录位置，便于推测模块信息，此处仅利用目录位置信息，不需要原始文件具体内容
filemaps={}
with open('testcases/testPath.txt') as f:
	flines=f.readlines()
for fline in flines:
	fline=fline.strip()
	ls=fline.split(' ')
...
#获取正确答案，便于评估demo效果
rightans={}
with open('testcases/expectedAPIs.txt') as f:
	rlines=f.readlines()
for rline in rlines:
	rline = rline.strip()
	ls=rline.split(' ')
...
active = True
while active:
	#print('Choose test cases\n [1]:alexnet/train.py##$hole$')
	print('=='*20)
	message = input('Choose test cases.Enter 1~5 \n [1] built-in API [2] standard API [3][4] mindspore API [5] self-defined API [quit] quit test\n')
	if message == 'quit':
		active = False
	elif message == '1':
		testnum='test1'
		CURRENT_FILE=filemaps[testnum]
		ifile='testcases/test1.py'
		all_mapis={}
		#记录别名
		alias_maps={}
		dealwith(ifile)
