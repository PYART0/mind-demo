...
with open('traincsv/data.csv','w+') as f:
	f.write(total_data)		
with open('traincsv/label.csv','w+') as f:
	f.write(total_label)


start=time.time()
data=pd.read_csv('traincsv/data.csv')
label=pd.read_csv('traincsv/label.csv')

labels=label.values.ravel()
clf=RandomForestClassifier()
clf.fit(data,labels)
result=clf.score(data,labels)
joblib.dump(clf,'traincsv/test.pkl')
print(result)
